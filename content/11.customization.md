...
Laraclaw has no timezone setting of its own. It follows your Laravel app, so set it once and everything lines up:

```env
APP_TIMEZONE=America/Argentina/Buenos_Aires
```

The agent gets the current time in that zone along with its UTC offset, so "remind me tomorrow at 10am" means ten in the morning where you are rather than ten UTC. The same zone is used for reminder scheduling, heartbeat cron expressions, and calendar events, and anything the agent reports back is converted into it first.

Leave `APP_TIMEZONE` unset and Laravel defaults to UTC, which is what Laraclaw will use.
